...
#define FSUDP "22120"	// the port users will be connecting to
#define DIRUDP1 "21120"
#define FSTCP "41120"  // the port users will be connecting to
...
int main(int argc, char *argv[])
{
	...
	int numbytes1;
	char buf1[50];
	int count3 =3; char phase1[20] =  "File_Server1 41120";
	...
	if(1)
	{
					...
					if ((rv1 = getaddrinfo(SERVERNAME, DIRUDP1, &hints1, &servinfo1)) != 0) {
						fprintf(stderr, "fileserver1: getaddrinfo: %s\n", gai_strerror(rv1));
						return 1;
					}
	...
	if(1)
	{
												...
												if (p == NULL)  {
													fprintf(stderr, "fileserver1: failed to bind\n");
													return 2;
												}
							 ...
							 printf("Phase3: File Server 1 has TCP port %d and IP address %s.\n",(int) ntohs(gfs.sin_port),inet_ntoa(gfs.sin_addr));
												 //  printf("fileserver1: Local IP address is: %s\n", inet_ntoa(gfs.sin_addr));
												//	printf("fileserver1: Local port is: %d\n", (int) ntohs(gfs.sin_port));

												freeaddrinfo(servinfo); // all done with this structure

												if (listen(sockfd, BACKLOG) == -1) {
													perror("fileserver1: listen\n");
													exit(1);
												}
												...
												//	perror("fileserver1: sigaction\n");
												//	exit(1);
												//}

												//printf("fileserver1: waiting for connections...\n");

												while(1)
												{  // main accept() loop
													/*if (setsockopt(new_fd, SOL_SOCKET, SO_REUSEADDR, &yes,
															sizeof(int)) == -1) {
														perror("fileserver1: setsockopt\n");
														exit(1);
													}*/
													...
													//printf("accept is a blocking call\n");
													if (new_fd == -1) {
														perror("fileserver1: accept\n");
														continue;
													}
												...
												//	printf("fileserver1: got connection from %s\n", s);
												//	printf("fileserver1: got connection from port is: %d\n", (int) ntohs((struct sockaddr_in *)&their_addr)->sin_port);

													if (!fork())
													{ // this is the child process
														close(sockfd); // child doesn't need the listener
														if ((numbytes = recv(new_fd, buf, MAXDATASIZE-1, 0)) == -1) {
														perror("fileserver1: recv\n");
														exit(1);
														}
														printf("Phase3: File Server 1 received the request from the %-.7s for the file %s.\n",buf, buf+8);
														//printf("fileserver1: received packet by fileserver from client %s\n", buf);
														if (send(new_fd, buf+8, 13, 0) == -1)
														perror("fileserver1: send\n");
