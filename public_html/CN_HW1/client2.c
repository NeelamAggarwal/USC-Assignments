/*
ALL CODE IS REUSED FROM client1 with only string changed
*/
...
#define CUDP "33120"	// the port users will be connecting to
...
int main(int argc, char *argv[])
{
	...
	socklen_t gc_len;
	struct hostent *he;
	int count2 =2; char phase2[20] =    "Client2 doc2"; int phaseidx=0;
	int sockfd, numbytes;  
	...
	if(1)
	{
						...
						if ((rv1 = getaddrinfo(SERVERNAME, DIRUDP2, &hints1, &servinfo1)) != 0) {
							fprintf(stderr, "client2: getaddrinfo: %s\n", gai_strerror(rv1));
							return 1;
						}
					...
					/*	printf("client2: getaddrinfo address %s\n",
					...
					printf("Phase2: Client 2 has UDP port number %d and IP address %s.\n",(int) ntohs(gc1.sin_port),inet_ntoa(gc1.sin_addr));
							...
							printf("Phase2: The File request from Client 2 has been sent to the Directory Server.\n");

							if ((numbytes1 = recvfrom(sockfd1, buf1, 20 , 0,
								(struct sockaddr *)&their_addr1, &addr_len1)) == -1) {
								perror("client2: recvfrom \n");
								exit(1);
							}
							//printf("client2: ackreceived: %s\n", buf1);
						printf("Phase2: The File requested by Client 2 is present in %-.12s and the File Server's TCP port number is %s.\n",buf1,buf1+13);

						freeaddrinfo(servinfo1);
						printf("Phase2: End of Phase 2 for Client 2.\n");
						//printf("client2: sent %d bytes to %s\n", numbytes1, phase2);
						close(sockfd1);
	}
	//strcpy(buf1, "File_Server1 48888");
	strcpy(portno, buf1+13);
	//printf("client2: tcp portno %s\n", portno);

	// UDP CONEECTION PHASE 3
	 if(1)
	{
														...
														if (p == NULL) {
															fprintf(stderr, "client2: failed to connect\n");
															return 2;
														}
					...
					printf("Phase3: Client 2 has dynamic TCP port number %d and IP address %s.\n",(int) ntohs(gc.sin_port),inet_ntoa(gc.sin_addr));

														inet_ntop(p->ai_family, get_in_addr((struct sockaddr *)p->ai_addr),
																s, sizeof s);
														/*printf("client2: connecting to %s\n", s);
										printf("directoryserver:got packet from port is: %d\n", (int) ntohs((struct sockaddr_in *)p->ai_addr)->sin_port);*/
														freeaddrinfo(servinfo); // all done with this structure

															if (send(sockfd,   "Client2 doc2", 13, 0) == -1)
															perror("client2: send\n");

														printf("Phase3: The File request from Client 2 has been sent to the %-.12s.\n", buf1);	
														...
														buf[numbytes] = '\0';
														printf("Phase3: Client 2 received %s from %-.12s.\n",buf,buf1 );
														

														//printf("client2: received '%s'\n",buf);
														printf("Phase3: End of Phase 3 for Client 2.\n");
														close(sockfd);
